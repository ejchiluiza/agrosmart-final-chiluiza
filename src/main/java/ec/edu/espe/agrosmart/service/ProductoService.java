...
import ec.edu.espe.agrosmart.exception.ProductoNoExisteError;
...
public class ProductoService {
    ...
    private static final Producto PRODUCTO_GENERICO = new Producto(
            0L, "Sin productos disponibles", "N/A", BigDecimal.ZERO, Collections.emptyList()
    );
    ...
    public Mono<Producto> buscarPorId(Long id) {
        return Mono.fromCallable(() -> repository.findById(id))
                ...
                .map(ProductoMapper::toDominio)
                .switchIfEmpty(Mono.error(new ProductoNoExisteError(id)));
    }
}
